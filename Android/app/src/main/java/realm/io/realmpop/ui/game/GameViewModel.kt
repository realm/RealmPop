...
import realm.io.realmpop.model.dao.isInvalid
...
class GameViewModel(application: Application) : AndroidViewModel(application), AnkoLogger, GameTimer.TimerDelegate {
    ...
    private var timer = GameTimer(GAME_LENGTH_SECONDS.toLong())
    ...
    fun onBubbleTap(numberTapped: Long) {
        ...
        if (numberThatShouldBeTapped == numberTapped) {
            gameDao.decrementBubbleCount(mySidePlayerId)

        } else {
            gameDao.failSide(mySidePlayerId, onSuccess = {
    ...
    private fun update() {
        ...
        // TODO: Break out timer updates here and move to DAO.
        realm.executeTransactionAsync(Realm.Transaction { bgRealm ->
            val currentGame = playerDao.byId(bgRealm, mySidePlayerId)?.currentGame!!
